  ...
  try {
    ...
    if (status) {
      ...
      conditions.push(
        inArray(
          documents.status,
          statuses as typeof documents.status.enumValues
        )
      )
    }
    ...
    if (category) {
      ...
      conditions.push(
        inArray(
          documents.category,
          categories as typeof documents.category.enumValues
        )
      )
    }
    ...
    const rolloutCountMap = new Map(
      rolloutCounts.map((r) => [r.documentId, r.count])
    )
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create the document record first (content will be persisted via the storage resolver)
    ...
    // Persist content via the storage resolver
    const contentToWrite = data.content ?? ""
    if (contentToWrite) {
      try {
        const orgSettings = await getOrgDocsSettings(session.orgId)
        await writeContent(created, orgSettings, contentToWrite, session.userId)
      } catch (err) {
        console.error("[Docs] Failed to write initial content:", err)
      }
    }
